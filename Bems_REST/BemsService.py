...
##실시간 공조기 temp data
async def GetAhuNowTempData(runDate: str):
    try:
        connection = GetConnection()
        ...
        with connection.cursor() as cursor:
            # query = "SELECT LpID, LpDate, cast(LpData as char) as LpData FROM wm_fems.Raw_KepcoDayLpData where LpDate > %s limit 2;"
            query = "select " +\
                "   cast(machine_num as char) as machine_num, runDateTime, " +\
                "   cast(ahu_sat as char) as ahu_sat , " +\
                "   cast(ahu_oat as char) as ahu_oat, " +\
                "   cast(ahu_mat as char) as ahu_mat, " +\
                "   cast(ahu_rat as char) as ahu_rat " +\
            ...
            # ress = str(rv)
            json_data = json.dumps(rv, indent=4)
            # print(f"{datetime.datetime.now()} : succeed to do 'GetAhuNowTempData('{runDate}')'")
            _logger.Info(f"succeed to do 'GetAhuNowTempData('{runDate}')'")
            return json_data

    except Exception as ex:
        # print(f"{datetime.datetime.now()} : error to do 'GetAhuNowTempData('{runDate}'")
        _logger.Info(f"error to do 'GetAhuNowTempData('{runDate}'")



## Boiler total power data
async def GetBoilerTotalData(runDate: str):
    try:
        connection = GetConnection()
        ...
        with connection.cursor() as cursor:
            ...
            # ress = str(rv)
            json_data = json.dumps(rv, indent=4)
            # print(f"{datetime.datetime.now()} : succeed to do 'GetBoilerTotalData('{runDate}')'")
            _logger.Info(f"succeed to do 'GetBoilerTotalData('{runDate}')'")
            return json_data

    except Exception as ex:
        # print(f"{datetime.datetime.now()} : error to do 'GetBoilerTotalData('{runDate}'")
        _logger.Info(f"error to do 'GetBoilerTotalData('{runDate}'")
...
async def GetChillerCwstData(runDate: str):
    try:
        connection = GetConnection()
